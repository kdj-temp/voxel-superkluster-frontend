...
const ProfileMenu = styledComponents(Menu)`
  border-radius: 3px;
  padding: 15px 20px;
...
const NotificationButton = styledComponents.button`
  position: absolute;
  right: 22% ;
  bottom: 2% ;
  width: 55% ;
...
const ProfileMenuIcon = styledComponents.div`
  display: flex;
  justify-content: center;
  align-items: center;
  width: 24px;
...
const Header = function ({ location , funcs  ,colormodesettle }) {
  ...
  useEffect(() => {
    ...
    const timerID = setInterval(() => {
      // console.log('royalty chain: ', chainId);
  ...
  useEffect(()=>{
    ...
    // console.log(funcs , colormodesettle , 'header mode');
  },[]) ;
  ...
  useEffect(() => {
    const getUserInfo = async (account) => {
      ...
      if (userAvatar) {
        ...
        const imgSrc = "/img/misc/avatar-1.png";
        setAvatarSrc(imgSrc);
      }
    }
  ...
  useEffect(()=>{
    // console.log(colormodesettle.ColorMode,'colormodesettle')  ;
    if(colormodesettle.ColorMode) setlogoSrc("/img/logo.PNG") ;
    else setlogoSrc("/img/logo2.png")
    if(width < 600) setlogoSrc("/img/mobile_logo.png");
  ...
  const handleKeyPress = (event) => {
    if(event.key === 'Enter'){
      // console.log(isSearchLabel,'isSearchLabel');
  ...
  const signOut = async () => {
    ...
    // localStorage.removeItem('approvedToken');
    navigate('/');
  }
  ...
  const AvatarIcon = props => {
    return <div {...props}>
      <span>
        <img src={avatarSrc?avatarSrc:"/img/misc/avatar-1.png"} alt="avatar" style={{ width: '100%', height: '100%', borderRadius: '50%' }} />
      </span>
    </div>;
  }
  ...
  const NotificationIcon = props => {
    return <div  {...props} >
      <span className="">
        {/* {console.log(isNotificationDisplay,'isNotificationDisplay' , localStorage.getItem('NotificationNewCnt'))} */}
        {/* <img src={avatarSrc} alt="avatar" style={{ width: '100%', height: '100%', borderRadius: '50%' }} /> */}
  ...
  const  handleClick=()=> {
    ...
    // and the updated count is emitted back via 'buttonUpdate'
      ...
      <ProfileMenu className="Non-scroll-tab">
        ...
        <Menu.Item key={2}>
          ...
          <div className="menu-text" style={{ display: 'flex' }}>
            <img src="/img/VXL.png" alt="balance" style={{ width: 18, height:18, margin: '0px 3px 3px 0px'}}/>
            {/* <Statistic className="balance-statistic" value={isBalance} precision={2} />{" "}(VXL) */}
        ...
        {(royalties > 0  || royaltiesETH > 0) && 
          <Menu.Item key={9}>
            <h5 className="pinkarr">Royalties</h5>
            {
              ...
              <img src="/img/VXL.png" alt="balance" style={{ width: 18, height: 19, margin: '0px 3px 3px 0px'}}/>
              {/* <Statistic className="balance-statistic" value={isBalance} precision={2} />{" "}(VXL) */}
            ...
            }
            {
              ...
              <div className="menu-text" style={{ display: 'flex' }}>
                <img src="/img/icons/ethIcon.png" alt="balance" style={{ width: 18, height: 19, margin: '0px 3px 3px 0px'}}/>
                {/* <Statistic className="balance-statistic" value={isBalance} precision={2} />{" "}(VXL) */}
        ...
        <Menu.Item key={8}>
          ...
          <a href="https://shibaswap.com/#/swap?outputCurrency=0x16CC8367055aE7e9157DBcB9d86Fd6CE82522b31" target="_blank" style={ProfileMenuStyle}>
            ...
            <img src="/img/VXL.png" alt="balance" style={{ width: 18, margin: '0px 3px 3px 0px'}}/>
            {/* </ProfileMenuIcon> */}
  ...
  const call_notification =  ()=>{
    ...
    Axios.post(
      ...
      // let cnt = localStorage.getItem('NotificationNewCnt') ;
      // if (cnt == null ) localStorage.setItem('NotificationNewCnt',sort_notification.length) ;
      // localStorage.setItem('NotificationNewCnt',sort_notification.length) ;
      // console.log(localStorage.getItem('NotificationNewCnt') ,'notification number');
      // setNotificationDisplay(sort_notification ? sort_notification.length - localStorage.getItem('NotificationNewCnt') : 0 );
      // console.log(resp.data.data , sort_notification ,'respdata')   ;
  ...
  useEffect( ()=>{
    ...
    // navigate('/explore', { state: {value: `` , val_flg : true} });
  ...
  const deleteNotification = async (notification_id) =>{
      ...
      await Axios.post(
        ...
        // console.log('remove selected notification') ;
        call_notification() ;
        // let cnt_notification = localStorage.getItem('NotificationNewCnt') ;
        // localStorage.setItem('NotificationNewCnt',parseInt(cnt_notification) - 1) ;
  ...
  const redirect_itemdetail = (id, n_id , n_type) =>{
    // if(n_type == "reject_verify" ||  n_type ==  "reject_verify_user") return ;
    // let temp = [] ;
    // // console.log(temp,'temp_detail') ;
    // for(let i = 0 ; i < isNotificationData.length ; i ++ ){
    //   if (n_id == isNotificationData[i].link) continue ;
    //   temp.push(isNotificationData[i]) ;
    // }
    // // console.log(temp) ;
  ...
  const NotificationMenuOverlay = () => {
    // console.log(localStorage.getItem('NotificationNewCnt') ,'notification number');
    // localStorage.setItem('NotificationNewCnt', isNotificationData ? isNotificationData.length: 0) ;
    // console.log(localStorage.getItem('NotificationNewCnt') ,'notification number');
      ...
      <>
        ...
        <div className="notificationDel" style={isNotificationData && isNotificationData.length == '0' ? {position:'relative',paddingBottom:'0px'}:{position:'relative'}}>
          <ProfileMenu className="notification-scroll" style={{width:'350px' , maxHeight:'400px' , overflowY:'scroll'}}>
            {/* {console.log(isNotificationData ,isNotificationData[0] ,'isnotificationdata')} */}
            {/* {console.log(isNotificationData,'isNotificationData')} */}
          ...
          {isNotificationData && isNotificationData.map((notification , inx)=>(
                <Menu.Item key={notification.id} >
                  <div style={{display:'flex', justifyContent:'space-between'}} onClick = {()=>redirect_itemdetail(notification.id, notification.link , notification.type)}>
                    {/* <Link to={`/ItemDetail/${notification.link}`}  style={ProfileMenuStyle}> */}
                    <Link to={
                      ...
                      >
                      <ProfileMenuIcon className="headerIconSy">
                      <LazyLoadImage effect="opacity" className="lazy card-img-top" src={notification.avatar ? notification.avatar: "/img/misc/avatar-1.png"} style={{ width: 35, height: 35, borderRadius: '50%' , marginRight:'10px' }} alt="Bologna" />
                      </ProfileMenuIcon>
  ...
  const cartData =[
    {
      ...
      image: '',
      price: 5,
      collection: 'VXL collection'
    },
    {
      id: 2,
      name: 'Lazy Monkey#434',
      image: '',
      price: 20,
      collection: 'Monkey collection'
    },
    {
      id: 3,
      name: 'Crypto Punk#1105',
      image: '',
      price: 105,
  ...
  const handleBatchBuy = async() => {
    ...
    // call buy-cart api to interact with contract
  ...
  const usd_price_set=(num)=>{
    ...
    let str = '' ;
    if(num > 1000) str = parseFloat(num / 1000).toFixed(2) + 'K' ;
    if(num > 1000000) str = parseFloat(parseInt(num / 10000)/100).toFixed(2) + 'M' ;
    if(num < 1000) str = num.toFixed(0) ;
    return str ;
  }
  const eth_price_set=(num)=>{
    // console.log(use_price , num,'ddddddddddddd')
    let str = '' ;
    if(num > 1000) str = parseFloat(num / 1000).toFixed(2) + 'K' ;
    if(num > 1000000) str = parseFloat(parseInt(num / 10000)/100).toFixed(2) + 'M' ;
    if(num < 1000) str = num.toFixed(3) ;
    return str ;
  }
  const usdPrice_num_usd=(num)=> {
    let str = '' ;
    if(num > 1000) str = parseFloat(parseInt(num / 100)/10).toFixed(2) + 'K' ;
        ...
        <div className="custom-container">
          <div className="row w-100-nav">
            <div className="logo px-0">
              {/* <ThemeToggleBtn/> */}
              {/* <button onClick={handleClick}>ddddd</button> */}
              <div className="navbar-title navbar-item">
                <NavLink to="/">
                  <img
                    ...
                    alt=""
                    style={window.innerWidth < 365 ? {height:'40px'}:{height:'50px'}}
                  />
                </NavLink>
              </div>
            </div>
                    ...
                    <div ref={ref1}>
                      <NavLink to="/explore">
                        <div
                          ...
                          style={linkBtnStyle}
                        >
                        Explore<span className="lines"></span>
                        </div>
                      </NavLink>
                    </div>
                  </div> */}
                        ...
                        {openMenu2 && (
                          <div className="item-dropdown">
                            <div className="dropdown" onClick={closeMenu2}>
                              {/* <NavLink to="/activity" style={{ borderBottom: '1px solid #dfdfdf', ...navLinkStyle }}><NaveMenuIcon><FaBacon style={navMenuIconStyle} /></NaveMenuIcon>Activity</NavLink> */}
              ...
              : !onSearch && <div className="mainside" style={{ top: 18, right: 30 }}>
                  <div className="connect-wal" >
                    <div style={signinDisplay}>
                      <NavLink to="/wallet" className="btn-main btn-wallet lead" style={{ width: '100%', padding: '8px 32px' }}>
                        <span>Sign in</span>
                      </NavLink>
                    </div>
            ...
            <div className="modal-content" style={{ borderColor: '#dee2e6', borderRadius: 15, width: '98%' }}>
              ...
              <Spin spinning={loadingState} indicator={antIcon} delay={500}>
                            ...
                            <RowDiv className="item_author justify-content-between" style={{alignItems:'center'}}>
                              <div className="d-flex" style={{cursor:'pointer', alignItems:'center', width:'70%', overflow:'hidden'}} onClick={(e) => goDetailPage(e, data.asset.id)}>
                                <RowAvatar className="author_list_pp">
                                  <span>
                                    <LazyLoadImage effect="opacity" className="lazy" src={data.asset.image ? data.asset.image : "/img/collections/coll-1.jpg"} alt="" />
                                  </span>
                                </RowAvatar>
                                <RowInfo className="author_list_info" style={{  paddingTop: 0, lineHeight: 1.2, display:'flex', alignItems:'center', width:'80%'}}>
                                  {/* <PTag className="menu-text">
                                    {data.name}
                                  </PTag> */}
                                  <div style={{width:'100%'}}>
                              ...
                              <div style={{display:'flex', justifyContent:'space-between', width:'30%', alignItems:'center'}}>
                                {/* <h4 style={{fontSize:'13px', marginRight:'0.6rem', marginTop:'0.6rem'}}><img style={{ width: 16, height: 16, marginBottom: 5 ,marginLeft: 0 }} src="/img/VXL.png" />  {(data.asset.price / usdPrice).toFixed(0)} </h4> <p style={{color:'grey', fontSize:'13px', marginTop:'0.6rem', marginRight:'0.6rem'}}>$ {data.asset.price}</p> */}
                                <DescriptionDiv><h5 style={{fontSize:'13.5px', marginBottom:'0px'}}><img style={{ width: 16, height: 16, marginBottom: 5 ,marginLeft: 0 }} src={ethOption? "/img/icons/ethIcon.png":"/img/VXL.png"} />
                                  {
                                    !ethOption?
                                    ` ${usd_price_set(data.asset.price / usdPrice)}`
                                    :
                                    ` ${eth_price_set(data.asset.price / ethPrice)}`
                                  }
                                </h5><h5 style={{fontSize:'13.5px'}}>&nbsp;$&nbsp;{usdPrice_num_usd(data.asset.price)}</h5></DescriptionDiv>
                ...
                <div className="toatlValueStyle" style={{borderRadius:'5px', margin:'0.5rem 1rem 0 1rem', padding:'0.5rem 0.5rem 0 0.5rem', display:'flex', justifyContent:'space-between'}}>
                  <h4>Total</h4>
                  <h4><img style={{ width: 23, height: 23, marginBottom: 5 ,marginLeft: 0 }} src={ethOption? "/img/icons/ethIcon.png":"/img/VXL.png"} />
                  {
                    !ethOption?
                    ` ${(totalCartPrice && usdPrice) ? usd_price_set(totalCartPrice / usdPrice):0} ($ ${usd_price_set(totalCartPrice)})`
                    :
                    ` ${(totalCartPrice && ethPrice) ? eth_price_set(totalCartPrice / ethPrice):0} ($ ${usd_price_set(totalCartPrice)})`
                  }
                  </h4>
                </div>
