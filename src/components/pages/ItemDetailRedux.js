...
import { Button, Affix, notification, Statistic, Checkbox, Switch, Select, Input,Collapse, DatePicker,Tooltip , Spin ,Progress ,  Dropdown, Menu, Space } from "antd";
import { LoadingOutlined } from '@ant-design/icons';
import { FaWallet, FaTag, FaHeart ,FaDollarSign ,FaInstagram,FaGlobe,FaFacebook,FaTwitter, 
  FaTelegram , FaSyncAlt ,FaShareAlt ,FaExternalLinkAlt, FaRegPaperPlane, FaArrowCircleRight, 
...
import NftCard from '../components/DetailNftCard';
...
import ThreeDtypeObjGltf from "./ThreeDTypeObjGltf";
import ThreeDOnline from "./ThreeDOnline";
import { FiInfo, FiClock, FiChevronLeft, FiChevronDown, FiCheckCircle, FiAlertTriangle } from 'react-icons/fi';
import {format , zonedTimeToUtc} from  'date-fns-tz' ;
import { useWeb3React } from "@web3-react/core";
import { Column, Line, Datum } from '@ant-design/plots';
...
const swalComp = (itemData)=>{
  return (
    <div><img style={{width:'220px' ,height:'auto', borderRadius:'5px' , marginBottom:'10px'}}  src = {itemData && (itemData.name )} /><div><div style={{marginTop:'25px' }}><span style={{fontWeight:'700' , fontSize:'21px' }}>Congratulations!</span><br/><br/> You now own <br/><span style={{color:'#f70dff',fontSize:'18px',fontWeight:'500'}}> { (itemData && itemData.owner)? itemData.owner.name :  itemData.owner_of.slice(0,5) + "..." + itemData.owner_of.slice(-5) }</span><br/><br/><span style={{fontSize:'13px'}}> Let's share the good news with the world!</span></div><div className='swalhtml'>
             
       <a href={itemData && itemData.owner && itemData.owner.link_instagram && itemData.owner.link_instagram != "https://" ? itemData.owner.link_instagram : "" }  style={ itemData && itemData.owner && itemData.owner.link_twitter && itemData.owner.link_twitter != "https://" ? {color:'#f70dff'} : {pointerEvents: 'none',color:'#f70dff'}} target="_blank"><div><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-instagram" viewBox="0 0 16 16">
      ...
      <a  href={itemData && itemData.owner && itemData.owner.link_external && itemData.owner.link_external != "https://" ? itemData.owner.link_external : "" } style={ itemData && itemData.owner && itemData.owner.link_twitter && itemData.owner.link_twitter != "https://" ? {color:'#f70dff'} : {pointerEvents: 'none',color:'#f70dff'}} target="_blank"><div><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-globe" viewBox="0 0 16 16">
      ...
      <a  href={itemData && itemData.owner && itemData.owner.link_twitter && itemData.owner.link_twitter != "https://" ? itemData.owner.link_twitter : "" }  style={ itemData && itemData.owner && itemData.owner.link_external && itemData.owner.link_external != "https://" ? {color:'#f70dff'} : {pointerEvents: 'none',color:'#f70dff'}} target="_blank"><div><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-globe" viewBox="0 0 16 16">
  ...
  return (
    <section className="custom-container" style={{ paddingTop: "25px", marginTop: "20px" }}>
      ...
      {/* <div className="mt-5">
        ...
        <div style={{ border: '1px solid #e5e8eb', borderTop: 'none', borderBottomLeftRadius: 10, borderBottomRightRadius: 10}}>
          <div style={{ padding: 25,  overflowY: "scroll", maxHeight: 400}}>
          ...
          <div style={{ padding: 10, textAlign: 'center', borderTop: '1px solid #e5e8eb'}}>
...
const ItemDetailRedux = ({ nftId ,colormodesettle }) => {
  ...
  const [isVerifyAState, setVerifyAState] = useState(false);
                      ...
                      <div 
                        ...
                        onClick={() => openUrl(
                          (itemData.asset_type === "video/mp4" || itemData.asset_type === "audio/mpeg" || itemData.asset_type === "3D") ?
                            itemData.animation
                            : (itemData.image? itemData.image : itemData.raw_image?itemData.raw_image:itemData._blob_raw_image?itemData._blob_raw_image:"/img/collections/coll-item-1.jpg"))}>
                      ...
                      <span style={{width:'100%'}}>
                        ...
                        (
                          ...
                          //  <StyledButton data-bs-toggle="modal" data-bs-target="#auction_buyModal" size="large"  onClick={()=>image_original_show(itemData.image)}>
                              <LazyLoadImage
                               effect="opacity"
                               src={/*itemData.image_preview&&itemData.asset_type !="image/gif" ? itemData.image_preview :*/ (itemData.image? itemData.image : itemData.raw_image?itemData.raw_image:itemData._blob_raw_image?itemData._blob_raw_image:"/img/collections/coll-item-1.jpg")}
                               className="img-nft-detail img-rod mb-sm-30"
                               data-bs-toggle="modal" data-bs-target="#imageOriginalModal" size="large"  onClick={()=>image_original_show(itemData.image? itemData.image : itemData.raw_image?itemData.raw_image:itemData._blob_raw_image?itemData._blob_raw_image:"/img/collections/coll-item-1.jpg")}    
                               alt=""
                             />
                            //  </StyledButton>
                        )
                      ...
                      { itemData.is_721 &&
                        <div className="mr40">
                          ...
                          <div className="item_author" style={{position:'relative'}} onClick={()=>authorCollection(itemData.owners? itemData.owners[0].owner.public_address: itemData.owner_of)}>
                            <div className="author_list_pp">
                              <span>
                                <LazyLoadImage effect="opacity" className="lazy" src={(itemData.owners.length > 0 && itemData.owners[0].owner.avatar) ? itemData.owners[0].owner.avatar : "/img/misc/avatar-1.png"} afterLoad={() => setLoadImgStatus(!loadImgStatus)} alt="" />
                                  {
                        ...
                        <div className="item_author" style={{position:'relative'}} onClick={() => moveToCollectionPage(itemData.collection.link)}>
                          <div className="author_list_pp" >
                            <Link to={"/collection-detail/" + itemData.collection.link}>
                              <LazyLoadImage effect="opacity" className="lazy" src={itemData.collection.avatar ? itemData.collection.avatar : "/img/collections/coll-1.jpg"} afterLoad={() => setLoadImgStatus(!loadImgStatus)} alt="" />
                        ...
                        <div style={{padding:'10px 20px', borderTop:!colormodesettle.ColorMode? 'solid 1px rgb(53, 56, 64)' : 'solid 1px #e3e3e3'}}>
                          <div className="tab-1 onStep fadeIn">
                            <div className="d-block mb-3">
                              ...
                              <div className="row">
                                {/* {console.log(itemData,'itemdata_detail')} */}
                                {isPropertiFlg 
                                  ?
                                    ...
                                    <div className="testIconBlockchain"></div>
                                      <div className="" style={{display:'flex' , justifyContent:'space-between' ,margin:'10px', alignItems: 'center'}}>
                                        {/* <div className="" onClick={()=>authorCollection(itemData.owner.public_address)}> */}
                                          ...
                                          {/* <div className="author_list_info" style={voxelNameFont}>
                                          <span className="NorTxt">Blockchain</span>
                                        </div> */}
                                        <div className="author_list_info" style={voxelNameFont}>
                                          <span className="NorTxt" style={{fontSize:'16px'}}>
                                            {
                                  ...
                                  <RowDiv className="item_author" style={{display:'flex' , justifyContent:'space-between', whiteSpace:'nowrap', marginBottom:'10px'}}>
                                    <div style={{display:'flex', marginTop:'10px'}}>
                                      <RowAvatar className="author_list_pp" onClick={()=>authorCollection(data.bidder && data.bidder.address)}>
                                        <span>
                                          <LazyLoadImage effect="opacity" className="lazy" src={data.bidder && data.bidder.avatar ? data.bidder.avatar : "/img/misc/avatar-1.png"} afterLoad={() => setLoadImgStatus(!loadImgStatus)} alt="" />
                                    ...
                                    }
                                    {
                                      itemData.auction_pay == true && data.bidder.address.toLowerCase() == account?.toLowerCase() && (itemData.auction_pay_end_date * 1000 >= new Date().getTime() && itemData.auction_buyer_pay_start_date * 1000 <= new Date().getTime()) ?
                                        ...
                                        // <ActionButton data-bs-toggle="modal" data-bs-target="#buyModal" className="btn-main lead mr15 style-btn" icon={<FaWallet style={iconStyle} />} disabled={ itemData.on_sale == true && accessToken && isOwnerBtnShow == false && itemData.sale_type !='2'  ? false : true } >Buy Now</ActionButton>
                                        : <></>
                                    }  
                                    </div>
                                    {/* auction_pay_end_date */}
                                      ...
                                      <span>
                                        {
                                          (data.activity === "mint" || data.activity === "sale")  ? 
                                            <LazyLoadImage effect="opacity" className="lazy" src={data.to.user_avatar ? data.to.user_avatar : "/img/misc/avatar-1.png"} afterLoad={() => setLoadImgStatus(!loadImgStatus)} alt="" />
                                          : <LazyLoadImage effect="opacity" className="lazy" src={data.from.user_avatar ? data.from.user_avatar : "/img/misc/avatar-1.png"} afterLoad={() => setLoadImgStatus(!loadImgStatus)} alt="" />
                                        }
                                        <i className="fa fa-check"></i>
                                      </span>
                                    </RowAvatar>
                          ...
                          : is_721 &&
                            <>
                              ...
                              </div>
                              {
                                ...
                                <div style={{ display: 'flex', alignItems: 'center' }}>
                                  <img src="/img/icons/ethIcon.png" alt="" width={25} style={{marginTop:'-10px'}} />
                                  <Statistic 
                                    style={{ fontColor: "#0d0c22", color: "#0d0c22", fontWeight: "bold", paddingRight: "10px", paddingLeft: "10px", marginBottom: "10px", fontSize: 28 }} 
                                    value={
                                      // itemData.
                                        ...
                                        <div className="d-flex flex-row">
                                          {
                                            ...
                                            <StyledButton data-bs-toggle="modal" data-bs-target="#placeBid" className="btn-main lead ItemBtnHover" onClick={getBidHistoryData} icon={<FaTag style={iconStyle} />} disabled={  !(accessToken && (isOwnerBtnShow == false && is_721 || !is_721))  || (itemData.auction_end_process == true && (new Date().getTime() >= itemData.auction_end_date * 1000 && new Date().getTime() <= itemData.auction_pay_end_date * 1000) || itemData.sale_type == 2 && new Date().getTime() <= itemData.auction_start_date * 1000) || isAuctionProcess == true || !is_721 && ownedSupplyNum == itemData.supply_number ? true : false } style={window.innerWidth>768?{width:'150px'}:{width:'100px'}}>{bidText}</StyledButton>
                                          }
                                          <StyledButton className="ItemBtnHover" style={window.innerWidth>768?{width:'150px'}:{width:'100px'}} onClick={() => moveToListingPage(itemData.id)}>Sell</StyledButton>
                                          <Tooltip placement="top"  title={auction_infoText_sell}>
                                              <FiInfo style={iconBtnStyle} />
                  ...
                  <div className="modal-body">
                    <Spin spinning={loadingState} indicator={antIcon} delay={500}>
                      ...
                      <div className="modal-top-bar">
                          ...
                          <ModalTopBarRight>
                              <div style={{ textAlign: 'right' }}>
                                <StyledStatistic className="vxlPrice" value={itemData.price && `${parseFloat(itemData.price / (ethOption? isETHPrice:isUSDPrice)).toFixed(ethOption? 3:0).toString()} ` } precision={0} prefix={<img style={{ width: 16, height: 16, marginBottom: 5 }} src={!ethOption? currencyLogo(itemData? itemData.chain_id : null): "/img/icons/ethIcon.png"} />} />
                      ...
                      <ModalTotalBar>
                        ...
                        <ModalTopBarRight>
                            <div style={{ textAlign: 'right' }}>
                              <StyledStatistic className="vxlPrice vxlTotalPrice vxlPriceColorFul" value={itemData.price && `${parseFloat(itemData.price / (ethOption? isETHPrice:isUSDPrice)).toFixed(ethOption? 3:0)}*` } precision={0} prefix={<img style={{ width: 16, height: 16, marginBottom: 5 }} src={!ethOption? currencyLogo(itemData? itemData.chain_id : null):'/img/icons/ethIcon.png'} />} />
                  ...
                  <div className="modal-body">
                    <Spin spinning={loadingState} indicator={antIcon} delay={500}>
                      ...
                      <div style={{ padding: '5px 0px 25px 0px'}}>
                        ...
                        <FlexDiv>
                          {/* <div style={{ width: '30%', margin: '0px 5px 0px 0px'}}>
                              <StyledSelect defaultValue="1" onChange="" disabled>
                                  <Option value='1'><StyledTokenImg src="/img/VXL.png" />VXL</Option>
                              </StyledSelect>
                          </div>
                          <div style={{ width: '70%', margin: '0px 0px 0px 5px' }}>
                            <StyledInput placeholder="Price" defaultValue={isBidPrice} onChange={handleChangeBidPrice} value={isBidPrice} />
                          </div> */}
                      ...
                      isOwnersData.map((data, index) => (
                        ...
                        <div className="mr40 mb-4 border-top-0" key={index} data-bs-dismiss="modal">
                          <RowDiv className="item_author justify-content-between color-blue" >
                            ...
                            <RowAvatar className="author_list_pp" onClick={()=>authorCollection_history(data)}>
                              <span>
                                <LazyLoadImage effect="opacity" className="lazy" src={data.owner.avatar ? data.owner.avatar : "/img/misc/avatar-1.png"} afterLoad={() => setLoadImgStatus(!loadImgStatus)} alt="" />
                                {
