...
import { Spin, Select, Tooltip, Button,Progress , notification ,Alert,Checkbox  ,Upload} from 'antd';
...
const CreatePage = (colormodesettle) => {
  ...
  // const [isRawAnimationMTL, setRawAnimationMTL] = useState(prop && prop.raw_animation ? prop.raw_animation : '')
  ...
  const [isTokenIcon, setTokenIcon] = useState("/img/VXL.png");
  ...
  const objwarning = <div className="text-center" style={{ maxWidth: 400 }}><span>please make sure all files have been downloaded<br/>Before minting your 3d model please ensure you verify and check your 3d model is displaying accurately at <a style={{color:'#f70dff'}} href="http://3dviewer.net/" target="_blank">http:/3dviewer.net</a> or <a style={{color:'#f70dff'}} href="https://threejs.org/editor/" target="_blank">https:/threejs.org/editor</a> if not will be viewable in black and white, sot to add colour you need to add texture files</span></div> ;
  const fbxwarning = <div className="text-center" style={{ maxWidth: 400 }}><span>please make sure all files have been downloaded<br/>Before minting please check your fbx file includes colour details, if not, please check at <a style={{color:'#f70dff'}} href="http://3dviewer.net/" target="_blank">http:/3dviewer.net</a> or <a style={{color:'#f70dff'}} href="https://threejs.org/editor/" target="_blank">https:/threejs.org/editor</a> if not will be viewable in black and white, sot to add colour you need to add texture files</span></div> ;
  const otherwarning = <div className="text-center" style={{ maxWidth: 400 }}><span>please make sure all files have been downloaded<br/>Before minting your 3d model please ensure you verify and check your 3d model is displaying accurately at <a style={{color:'#f70dff'}} href="http://3dviewer.net/" target="_blank">http:/3dviewer.net</a> or <a style={{color:'#f70dff'}} href="https://threejs.org/editor/" target="_blank">https:/threejs.org/editor</a> if not will be viewable in black and white, sot to add colour you need to add texture files</span></div> ;
  ...
  useEffect(()=>{
    ...
    // if(isThreeDOpt == 'obj') setBeforeUploadingMsg(objwarning) ;
    // if(isThreeDOpt == 'fbx') setBeforeUploadingMsg(fbxwarning) ;
    // if(isThreeDOpt == 'glb' || isThreeDOpt == 'gltf') setBeforeUploadingMsg(otherwarning) ;

  },[isThreeDOpt]) ;

  useEffect(()=>{
    // console.log(imageSrc_preview,'imageSrc_preview',isNftType);
  },[imageSrc_preview]);
  ...
  const blockchainData = [
    {
        chainId: 5,
        name: 'Goerli-Testnet',
        logo: '/img/blockchain/goerli.png'
    },
    {
        chainId: 1,
        name: 'Ethereum',
        logo: '/img/blockchain/ethereum.png'
    },
    // {
    //     chainId: 137,
    //     name: 'Polygon',
    //     logo: '/img/blockchain/polygon.png'
    // },
    // {
    //     chainId: 42161,
    //     name: 'Arbitrum',
    //     logo: '/img/blockchain/arbitrum.png'
    // },
    // {
    //     chainId: 56,
    //     name: 'Binance Smart Chain',
    //     logo: '/img/blockchain/binance.png'
    // },
    // {
    //     chainId: 250,
    //     name: 'Fantom',
    //     logo: '/img/blockchain/fantom.png'
    // },
    // {
    //     chainId: 43114,
    //     name: 'Avalanche',
    //     logo: '/img/blockchain/avalanche.png'
    // }
  ];
  ...
  const sendData = () => {
    ...
    // console.log(isRawAnimation,'-raw' , isRawAnimationMTL ,'-mtl',isRawAnimationTexture , 'isSelectCollectionId') ;
    // console.log(isForm , isMTLForm , isTextureForm,'assets');
    setLoadingSpin(true);
    ...
    // console.log(createItems,'createItems_data') ;
  ...
  const getAuthorInfoData = async () => {
    ...
    // setESChecked(author_info.data.is_sensitive);
    ...
    try {
      ...
      if (cid) {
        const data = `https://ipfs.io/ipfs/${cid}`;
          ...
          // console.log(data,'israwAnimation') ;
      }
    } catch (error) {
...
useEffect(()=>{
  ...
  let addingUrl = '' ;
  isTextureForm.forEach( file =>{
    // console.log(isMTLAsolutePath , file.webkitRelativePath,'dddddddd');
    texturePathArray = file.webkitRelativePath.split('/') ;
    // console.log(texturePathArray , mtlPathArray);
    addingUrl = '' ;
    let i = 0;
    while (1) {
    ...
    // addingUrl = addingUrl+'/' ;
    // console.log(addingUrl,'addingUrl') ;
      ...
      let textureAsolutePath = "" ;
        ...
        if(check_type(file.name).toLowerCase() == 'obj' || check_type(file.name).toLowerCase() == 'fbx' || check_type(file.name).toLowerCase() == 'glb' || check_type(file.name).toLowerCase() == 'gltf'){
          ...
          if(check_type(file.name).toLowerCase() != isThreeDOpt){
            // notification.error({
            //   message: `please check uploading file type should be ${check_type(file.name).toLowerCase()} file.`
            // });
            Swal.fire({
              title: 'Warning!',
...
customClass: 'swal-height'
          ...
          // console.log(mtlAsolutePath,'mtlAsolutePath');
        }else {
  ...
  const props1 = {
    ...
    beforeUpload: (file,filename) => {
      ...
      if(!isAccept) {
        // notification.error({
        //   message: `please check uploading file type should be ${isThreeDOpt} file.`
        // });
        Swal.fire({
          title: 'Warning!',
...
customClass: 'swal-height'
        ...
        // console.log(file,'file detail',check_type(file.name).toLowerCase(), isThreeDOpt);
        if(check_type(file.name).toLowerCase() == isThreeDOpt ) ;
        ...
        // else if(check_type(file.name) == 'fbx' || check_type(file.name) == 'FBX'  ) {
        //   setMtlOption(true) ;
        //   setTextureOption(true) ;
        // }
        // else {
        ...
        // }
        setFileListMain([fileListMain, file]) ;
        // console.log(file , 'ddsss',filename , URL.toString(file)) ;
        setForm(file) ;
        add_ipfs_main_raw_animation(file) ;
      }
    ...
    try {
      ...
      if (cid) {
        const data = `https://ipfs.io/ipfs/${cid}`;
          ...
          // console.log(data,'israwAnimationMTL') ;
      }
      } catch (error) {
        // console.log('Error uploading file: ', error)
      } 
  ...
  const props2 = {
    ...
    beforeUpload: file => {
      ...
      if(!isAccept) {
        // notification.error({
        //   message: `please check uploading file type should be MTL/BIN file.`
        // });
        Swal.fire({
          title: 'Warning!',
...
customClass: 'swal-height'
        ...
        // console.log(check_type(file.name),'file detail');
        // if(check_type(file.name)=='mtl' || check_type(file.name)=='MTL' || check_type(file.name)=='bin' || check_type(file.name)=='BIN')
        // {
           setTextureOption(true) ;
           setMTLForm(file);
           add_ipfs_MTL_raw_animation(file) ;
  ...
  const add_ipfs_Texture_raw_animation = async(element)=>{
    try {
      ...
      if (cid) {
        const data = `https://ipfs.io/ipfs/${cid}`;
          // let tmp = [...isRawAnimationTexture] ;
          // tmp.push(data);                      
          setRawAnimationTexture((isRawAnimationTexture)=>[...isRawAnimationTexture , data]) ;
          // setTextureForm((isTextureForm)=>[...isTextureForm , element]) ;
          // console.log(data,'israwAnimationTexture') ;
  ...
  const props3 = {
    ...
    beforeUpload: file => {
      ...
      // console.log(isTextureForm,file,'file',file.URL() , file.fileList());
  ...
  const handleImageSelect_threeD = async (e) => {
    ...
    // console.log(compressedFile ,compressedFile.size ,'eheh') ;
  ...
  const handleImageSelect = async (e) => {
    ...
    // console.log(file,'files - normal',isNftType,URL.createObjectURL(file)) ;
    ...
    if(file.size >= limit_size) {
      // notification.error({
      //   message: "Max size of file - 100MB"
      // });
      Swal.fire({
        title: 'Warning!',
    ...
    if(file.type == ""){
      setLoading(false);
      // notification.error({
      //   message: "3D file upload failed.  please click 3D toggle to upload 3D file."
      // });
      Swal.fire({
        title: 'Warning!',
    ...
    // console.log('file upload' , isNftType) ;
    if (isNftType === 'video/mp4' || isNftType === 'audio/mpeg') {
      ...
      if(isNftType == "" ) {
        ...
        // setNftType(file.type);
        setImageSrc(URL.createObjectURL(file)) ;
        // console.log(URL.createObjectURL(file),'URL.createObjectURL(file)',imageSrc);
        setForm(file) ;
  ...
  const uploadToIpfs = async (file) => {
    ...
    // console.log( isRawAnimation ,'raw-animation' ,isRawImage , 'raw image') ;
  }
  ...
  const inputNumValidation = (e, param) => {
    ...
    if (e.target.value === 0 || e.target.value === '' || re.test(e.target.value)) {
      ...
      // if(param == "f_num") setFirstNum(e.target.value)
      // if(param == "s_num") setSecondNum(e.target.value)
    }
  }
  ...
  const networkOpts = [
    { value: 5, label: "Goerli", chainId: 5 },
  ...
  const collectionTooltipText = <div className="p-3 text-center" style={{ maxWidth: 400 }}><span>Moving items to a different collection may take up to 30 minutes. You can <a href="#" target="_blank">manage your collections here.</a></span></div> ;
  const esContentTooltipText = <div className="p-3 text-center" style={{ maxWidth: 400 }}><span>Setting your asset as explicit and sensitive content, like pornography and other not safe for work (NSFW) content, will protect users with safe search while browsing <b>SuperKluster</b>. <br /><a href="#" target="_blank">Learn more about explicit content at OpenSea here.</a></span></div>;
  const freezeTooltipText = <div className="p-3 text-center" style={{ maxWidth: 400 }}><span>Once locked, your content cannot be edited or removed as it is permanently stored in decentralized file storage, which will be accessible for other clients to view and use. <a href="#" target="_blank">Learn more about freezing metadata here</a>.</span></div>;
                  ...
                  <div className="">
                      ...
                      
                    {
                      ...
                      <div>
                          ...
                          {/* <Alert message={beforeUploadingMsg} type="info" closeText="Close" /> */}
                      ...
                      //      <InboxOutlined />
                      //    </p>
                      //    <p className="ant-upload-text">Drag and drop 3D file/s such as OBJ, MTL, textures to upload</p>
                      //    <p className="ant-upload-hint">
                      //      You can upload multi-file...
                      //    </p>
                      //  </Dragger>
                      :
                    ...
                    {/* <h5>Price</h5>
                    <input type="number" name="price" id="price" step={0.1} onChange={(e) => setPrice(e.target.value)} value={isPrice ?? ""} aria-label="price" className="form-control mt-2" />

                    <div className="spacer-10"></div> */}
                    <div className="col-10" style={{ display: 'flex' }}>
                      <i className="fa fa-archive" style={iconStyle}></i>

                      <div><h5>NFT Collection</h5></div></div>
                    <span>
                    Select the collection where your NFT will appear.
                      {/* <Tooltip placement="top" title={collectionTooltipText}>
                          <FiInfo style={iconBtnStyle} />
                      </Tooltip> */}
                    </span>
                    ...
                    <div className='dropdownSelect'>
                      {/* {console.log(propForAdd ,'propforadd')} */}
                      {
                        propForAdd &&
                    ...
                    <div className="row align-items-center">
                      ...
                      <div className="col-2" style={{ textAlign: 'right' }}>
                        <Switch onChange={handleUnlockChange} checked={unlockChecked} onColor="#f70dff" />
                      </div>
                    </div>
                    ...
                    <div className="row align-items-center">
                      <div className="col-10" style={{ display: 'flex' }}>
                        ...
                        <div>
                          ...
                          <Tooltip placement="top" title={esContentTooltipText}>
                              <FiInfo style={iconBtnStyle} />
                          </Tooltip> */}
                        </div>
                      </div>
                    ...
                    {/* <IconBtn type="button" className="createModelBtn" data-bs-toggle="modal" data-bs-target="#supply"  disabled={(isNetworkOpt == "Polygon") ? false : true} ><FiInfo style={iconBtnStyle} /></IconBtn> */}
                    ...
                    {/* <h5 style={{ display: 'inline-block' }}>Freeze metadata </h5>
                    <Tooltip placement="top" title={freezeTooltipText}>
                        <FiInfo style={iconBtnStyle} />
                    </Tooltip><br />
                    <span>Freezing your metadata will allow you to permanently lock and store all of this item's content in decentralized file storage.</span>
                    <input type="text" name="item_supply" id="item_supply" className="form-control mt-2" defaultValue="To freeze your metadata, you must create your item first." disabled /> */}
                    {console.log(validRoyalty)}
                  </div>
                </Form>
              </FormikProvider>
            </div>
            ...
            <div className="col-lg-3 col-sm-6 col-xs-12">
              ...
              <div className="nft__item m-0 card-box-shadow-style">
                ...
                <div className="author_list_pp">
                  <span>
                    <img className="lazy" src={isAvatarSrc ? isAvatarSrc : "/img/misc/avatar-1.png"} alt="" />
                    <i className="fa fa-check"></i>
                  </span>
                </div>
                <div className="nft__item_wrap">
                  <span>
                    { isNftType.includes("image") && <img src={imageSrc_preview ? imageSrc_preview : "/img/collections/coll-item-1.jpg"} id="get_file_2" className="lazy nft__item_preview" alt="" /> }
                    { isNftType == "video/mp4" && <img src={isOtherTypeSrc ? isOtherTypeSrc : "/img/collections/coll-item-1.jpg"} id="get_file_2" className="lazy nft__item_preview" alt="" /> }
                    { isNftType == "audio/mpeg" && <img src={isOtherTypeSrc ? isOtherTypeSrc : "/img/collections/coll-item-1.jpg"} id="get_file_2" className="lazy nft__item_preview" alt="" /> }
                    { isNftType == "3D" && <img src={isOtherTypeSrc ? isOtherTypeSrc : "/img/collections/coll-item-1.jpg"} id="get_file_2" className="lazy nft__item_preview" alt="" /> }
                  </span>
                </div>
