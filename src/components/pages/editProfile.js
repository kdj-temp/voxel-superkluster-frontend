...
import { Input, Button, Upload, Form,  Select, Alert, notification ,Tooltip} from "antd";
...
const IconContent = styled.i`
    ...
    margin-right:10px ;
`;
// {fontSize:'20px' , display:'flex' , alignItems:'center' ,marginRight:'10px'}
...
const EditProfile = ({funcs, colormodesettle}) => {
  ...
  const [UserNameAlert  , setUserNameAlert] = useState( "Please input your username!") ;
...
//  validation input form end
  ...
  const CheckingCorrectUsername =  (user_name)=>{
    ...
    for(let i = 0 ; i < cnt ; i ++ ) {
      if(user_name[i] == ' ') {
        // notification.error({
        //   message: "It is not allowed to have space in the User name." 
        // });
        return false ;
      }
    }
    ...
    Axios.post(
        ...
        if(resp.data.duplicate) {
          // notification.error({
          //   message: "User name is Duplicated" 
          // });
          return false ;
        }
      }).catch(err => {
  ...
  useEffect(() => {
    console.log('origin data: ', org_data);
    ...
    async function getProfile() {
      ...
      try {
        ...
        if (data) {
          ...
          // localStorage.setItem('profile_verification' , data['verify_request']) ;
  ...
  const handleUpdateProfile = (info) => {
    // updateUser(values);
    // console.log(CheckingCorrectUsername(u_name) , 'CheckingCorrectUsername');
    // return ;
     ...
     Axios.post(`/api/users/verify-request` , postData,
      {
        ...
        //   message: "Admin will verify your profile details. please wait."
        // });
        Swal.fire({
          title: 'It worked!',
      ...
      // console.log(info.telegramUsername , info.myProject ,'senddata') ;
      
    }
    // if (verfiyProfileOptionValue === 2) {
    //   const postData = {
    //     kyc: info.telegramUsername,
    //     verify_type:verfiyProfileOptionValue
    //   }
    ...
    //       notification.success({
    //         message: "Profile Verification successfully!"
    //       });
    //     }).catch(err => {
    ...
    Axios.post(`/api/users/profile` , formData,
      {
        ...
        // localStorage.setItem("updating", "1");
      ...
      if (info.file) {
        ...
        image.onload = () => {
          ...
          if(image.width < 1400 || image.width > 2000 || image.height < 280 || image.height > 320) {
            // `notification`.error({
            //   message: "Please check the dimensions of the image you are trying to upload."
            // });
            Swal.fire({
              title: 'Heads up',
                                ...
                                <ProfileForm
                                  ...
                                  <div id="collection">
                                      ...
                                      <StyledSelect
                                          ...
                                          //  = {}
                                          //disabled = {verificationStatu == '1' || verificationResult == true }
                                      >
                                        {/* {console.log("verifyProfileOptions",verifyProfileOptions ,verificationType)}  */}
                                          {
