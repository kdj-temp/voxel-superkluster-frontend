...
import { notification, Tabs, Button, Pagination, Spin, Input, Select, Empty, Checkbox } from 'antd';
import { FaTh, FaBacon, FaTelegramPlane, FaFacebookF, FaTwitter, FaInstagram, FaDiscord, FaGlobe, FaPen, FaPlus, FaSearch, FaFilter } from 'react-icons/fa';
//add new from Adam
import Activity from "../components/Activity";
import { FaHeart , FaDollarSign, FaAngleUp, FaAngleDown } from "react-icons/fa";
...
`;
const DiableButton = styled(Button)`
  color: black;
...
const CollectionItem = function ({ collectionId , colormodesettle}) {
  ...
  const sendData = async () => {
    ...
    // console.log(isSelectCollectionId);
    ...
    isPropertyInputData.map((properti,inx)=>{
        if(properti.name !=""){
          // formData.append('properties_trait_type[]',properti.name) ;
          property_type.push(properti.type);
          // formData.append('properties_value[]',properti.type) ;
  ...
  // });
    ...
    .then(async(data) => {
      // call pre-reveal sign backend api
      const sendData = {
  ...
  const getCollectionDetail = async () => {
    ...
    // console.log(result.data.usdPrice , 'usdprice') ;
  ...
  const verifyCollection = async ()=>{
    ...
    await Axios.post("/api/collections/request-verify", postData, { headers: header })
      .then((res) => {
        ...
        // notification['info']({
        //   message: 'Admin will verify your collection against your profile details please wait.',
        // });
        Swal.fire({
          title: 'Heads up',
      ...
      .catch((err) => { 
        // notification['error']({
        //   message: 'unfortunately we were unable to verify your collection, please resubmit after updating your profile details',
        // });
        Swal.fire({
          title: 'Oops...',
          text: err.response.data.msg,
  ...
  return (
    <>
      <div>
        ...
        <section className='jumbotron breadcumb no-bg' style={{ backgroundImage: `url(${ isDetailData.banner ? isDetailData.banner : "/img/background/AdobeStock_487850137.jpg" })`, backgroundSize:'cover', backgroundRepeat:'no-repeat', backgroundPosition:'center center'}} >
              ...
              <div className="d_profile" style={{ marginBottom: 0 }}>
                <div className="profile_avatar">
                  <div className="d_profile_img">
                    <img src={isDetailData && isDetailData.avatar ? isDetailData.avatar : "/img/collections/coll-1.jpg"} alt="avatar" />
                ...
                }
                {
                  ...
                  <>
                      <div style={{ position: 'absolute', left: 10, top: 20 }}>
                      {
                        ...
                        <DiableButton  onClick={verifyCollection}>
                          {
                            isAddItemBtnState ? 
                              "Pending"
                        ...
                        <div className={colormodesettle.ColorMode? 'filter-pad-light':'filter-pad-dark'} style={{overflow:'scroll'}}>
                          ...
                          {/* <div className={colormodesettle.ColorMode? 'filter-item-light':'filter-item-dark'}>
                            <div className="filter-item-title">
                              <span style={{fontWeight:'bold', fontSize:16}}>Currency</span>
                              {
                                <FaAngleDown />
                              }
                            </div>
