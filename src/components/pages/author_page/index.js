...
const AuthorPage = ({ authorId ,colormodesettle}) => {
    ...
    return (
      <ColumeGroup onClick={() => handleMoveDetailitem(content && content.asset? content.asset.id:0)} style={{width:'auto'}}>
        <div className="author_list_pp" style={{ width: 40, height: 40, position:'relative' }}>
          <span>
              <img effect="opacity" src={content && content.asset && content.asset.image ? content.asset.image : "/img/collections/coll-item-1.jpg" } alt="avatar" />
          </span>
        </div>
    ...
    return(
      <div className="item_author" style={{position:'relative', cursor:'pointer'}} onClick={()=>authorCollection(user.address)}>
        <div className="author_list_pp" style={{cursor:'pointer'}}>
          <span>
            <LazyLoadImage effect="opacity" className="lazy" src={user && user.avatar ? user.avatar:"/img/misc/avatar-1.png"} alt="" />
          </span>
        </div>
      ...
      render: content => (
        ...
        <div>
          <div style={{display:'flex', justifyContent:'left'}}>
            <img src={content.currency=='eth'? "/img/blockchain/ethereum.png":"/img/VXL.png"} alt="balance" style={{ width: 18, height:18, margin: '0px 3px 3px 0px'}}/>
  ...
  const columnsSell = [
    ...
    
    {
      ...
      render: content => <><span><img style={{ width: 16, height: 16, marginBottom: 5 ,marginLeft: -5 }} src="/img/VXL.png" />&nbsp;{usd_price_set(content/localStorage.getItem('usdPrice'))}</span><br/><span>${usdPrice_num_usd(content)}</span></> 
    },
    {
      title: 'Time',
      dataIndex: 'bid_date',
  ...
  const columnsFollowers = [
    {
      ...
      render: (content) => <div className="item_author" style = {{display: 'flex', justifyContent: 'flex-start', alignItems: 'center'}} onClick={() => handleMoveAuthorPage(content.follower.public_address)}>
            <div className="author_list_pp">
              <span>
                  <img effect="opacity" src={content.follower.avatar ? content.follower.avatar : "/img/profile.png" } alt="avatar" />
              </span>
            </div>
  ...
  const columnsFollowings = [
    {
      ...
      render: (content) => <div className="item_author" style = {{display: 'flex', justifyContent: 'flex-start', alignItems: 'center'}} onClick={() => handleMoveAuthorPage(content.following.public_address)}>
            <div className="author_list_pp">
              <span>
                  <img effect="opacity" src={content.following.avatar ? content.following.avatar : "/img/profile.png" } alt="avatar" />
              </span>
            </div>
            <div className="author_list_info" style = {{paddingLeft: '50px'}}>
