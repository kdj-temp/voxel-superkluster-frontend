...
const RankingRedux = ({colormodesettle}) => {
    ...
    return (
      <ColumeGroup onClick={() => handleRouteToCollection(content.collectionLink)}>
        <div className="author_list_pp" style={{ width: 40, height: 40, position:'relative' }}>
          <span>
              <img effect="opacity" src={content && content.collectionAvatar ? content.collectionAvatar : "/img/collections/coll-1.jpg" } alt="avatar" />
  ...
  const UserAvatar = ({creatorAddr, content}) => {
    return (
      <ColumeGroup onClick={() => handleRouteToUser(content.creator? content.creator.public_address : (creatorAddr? creatorAddr : "0x0"))}>
        <div className="author_list_pp" style={{ width: 40, height: 40, position:'relative' }}>
          <span>
              <img effect="opacity" src={content.creator && content.creator.avatar ? content.creator.avatar : "/img/misc/avatar-1.png" } alt="avatar" />
              {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (volume, content) => <DescriptionDiv><span><img style={{ width: 16, height: 16, marginBottom: 5 ,marginLeft: 0 }} src="/img/VXL.png" />&nbsp;{usd_price_set(content.volume)}</span><br/>&nbsp;<span>$&nbsp;{usdPrice_num_usd(content.usdVolume)}</span></DescriptionDiv>,
sorter: {
    ...
    },
    {
      ...
      render: content => <DescriptionDiv><span><img style={{ width: 16, height: 16, marginBottom: 5 ,marginLeft: 0 }} src="/img/VXL.png" />&nbsp;{usd_price_set(content/UsdPrice)}</span><br/>&nbsp;<span>$&nbsp;{usdPrice_num_usd(content)}</span></DescriptionDiv>
    },
    {
      title: 'Owners',
      dataIndex: 'owners',
          ...
          <div className='row'>
            <div className='col-lg-12'>
              {/* <div className="items_filter centerEl">
                <div className='dropdownSelect one'><Select className='select1' styles={customStyles} menucustom-containerStyle={{'zIndex': 999}} defaultValue={options[0]} options={options} /></div>
