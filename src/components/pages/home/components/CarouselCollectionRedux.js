          ...
          { isHotCollections && isHotCollections.map((item, index) => (
            <CollectionDiv className='card-box-shadow-style' style={{borderRadius:'20px'}} onClick={() => moveToItemPage(item.collectionLink)} key={index} index={index + 1}>
                <CardDiv className="card" style={{ height: 'auto' }}>
                    <LazyLoadImage effect="opacity" className="lazy card-img-top" src={item.collectionImg ? item.collectionImg : "/img/collections/coll-1.jpg"} style={{ aspectRatio:'8/5', objectFit:'cover', objectPosition:'top'}} alt="Bologna" />
                    <div style={{textAlign:'center',marginTop: -20 , marginBottom: 10}}>
                      <RowAvatar className="author_list_pp">
                        <div>
                            <LazyLoadImage effect="opacity" className="lazy card-img-top" src={item.creatorAvatar ? item.creatorAvatar : "/img/misc/avatar-1.png"} style={{ width: 50, height: 50, borderRadius: '50%', border: '2px solid white' }} alt="Bologna" />
