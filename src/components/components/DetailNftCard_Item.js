          ...
          {loadingState && <SkeletonCard />}
          {
            ...
            <div className="nft__item m-0 nftcardpage" style={{position: 'relative' ,paddingRight:'4px',paddingLeft:'4px',paddingTop:'0px', height:'100%' }}>
              ...
              <a className="author_list_pp hoverauthor" style={{ width: 40, height: 40,marginTop:'6px' ,marginLeft:'6px' }} href = {`collection-detail/${nft.collection.link}`}>
                <span>
                    <LazyLoadImage effect="opacity" src={nft && nft.collection.avatar ? nft.collection.avatar : "/img/collections/coll-1.jpg" } afterLoad={() => setLoadImgStatus(true)} alt="" />
              ...
              <div className="nft__item_wrap" style={{marginTop:'3px'}}>
                <Outer>
                  <span>
                    <LazyLoadImage
                      effect="opacity"
                      src={nft.asset.image_preview ? nft.asset.image_preview :  (nft.asset.image ? nft.asset.image : nft.asset.blob_raw_image? nft.asset.blob_raw_image:"/img/collections/coll-item-1.jpg")}
              ...
              <div className="nft__item_info nft_card_sub nft_card_flex_end" style={(get_remain_date_newTemplate(AuctionEndDate) != '0' && nft.asset.auctionStartDate * 1000 <= new Date().getTime()) ? {paddingRight:'19px',paddingLeft:'19px'}:{paddingRight:'19px',paddingLeft:'19px'  }}>
                ...
                {/* <CardDescription className="has_offers">
                  ...
                  {/* {console.log(nft.id, nft.asset.auctionEndDate * 1000  , new Date().getTime() , nft.asset.auctionStartDate * 1000 )} */}
                ...
                {/* sssssssssssssssssss */}
                {
                    nft.asset && nft.asset.saleType == 2 && nft.asset.auction_end_process && nft.asset.auction_end_process == true && (nft.asset && nft.asset.top_bid && nft.asset.top_bid != "null") ?
                      <CardBottom >
                        ...
                        {nft.asset && nft.asset.top_bid && nft.asset.top_bid ? <><div className="usd_color">${usd_price_set_usd(nft.asset.top_bid)} </div></> : <></>}
                            {nft.asset && nft.asset.top_bid && nft.asset.top_bid != "null" && <StyledStatistic style={{ margin: 0 }} value={usd_price_set(nft.asset.top_bid / localStorage.getItem('usdPrice'))}  prefix={<img style={{ width: 16, height: 16, marginBottom: 5 }} src={currencyLogo(nft? nft.chain_id : null)} />} />}
                    ...
                    nft.asset && nft.asset.saleType == 2 && nft.asset.auctionEndDate * 1000 >= new Date().getTime() ?
                      ...
                      <CardBottom >
                          ...
                          <div   style={{ width: 'max-content' }}>
                              ...
                              <div className="nItemSale bid-value mobile-margin" >
                                {nft.asset && nft.asset.top_bid && nft.asset.top_bid != "null" ? <StyledStatistic style={{ margin: 0 }} value={usd_price_set(nft.asset.top_bid/localStorage.getItem('usdPrice'))}  prefix={<img style={{ width: 16, height: 16, marginBottom: 5 }} src={currencyLogo(nft? nft.chain_id : null)} />} /> : <StyledStatistic style={{ margin: 0 }} value={usd_price_set(nft.asset.price/localStorage.getItem('usdPrice'))}  prefix={<img style={{ width: 16, height: 16, marginBottom: 5 }} src={currencyLogo(nft? nft.chain_id : null)} />} /> }
                                {nft.asset && nft.asset.top_bid && nft.asset.top_bid != "null" ? <><div className="usd_color"><img style={{ width: 16, height: 16, marginBottom: 5 }} src="/img/icons/ethIcon.png" /> {`${usd_price_set_eth(nft.asset.top_bid)} ($${usd_price_set_usd(nft.asset.to_bid)})`} </div></> : <><div className="usd_color"><img style={{ width: 16, height: 16, marginBottom: 5 }} src="/img/icons/ethIcon.png" /> {`${usd_price_set_eth(nft.asset.price)} ($${usd_price_set_usd(nft.asset.price)})`} </div></>}
                              </div>
                          </div>
                      </CardBottom>
                      :
                     ...
                     <CardBottom >
                       <div className="nItemSale" style={{ width: 'max-content', minHeight: 80 }}>
                           ...
                           {nft && nft.asset && nft.asset.price && nft.asset.price != "null" && nft.asset.price > 0 ? <StyledStatistic style={{ margin: 0 }} value={usd_price_set(nft.asset.price / localStorage.getItem('usdPrice'))}  prefix={<img style={{ width: 13, height: 16, marginBottom: 5 }} src={currencyLogo(nft? nft.chain_id : null)} />} /> : <><div style={{ fontSize: 12}}>{nft.asset.onSale ?"Free Item":"Not for sale" }<div style={{ width: 16, height: 16 }}></div></div></>}
                            {nft && nft.asset && nft.asset.price && nft.asset.price != "null"  && nft.asset.price > 0  ? <><div className="usd_color"><img style={{ width: 16, height: 16, marginBottom: 5 }} src="/img/icons/ethIcon.png" /> {`${usd_price_set_eth(nft.asset.price)} ($${usd_price_set_usd(nft.asset.price)})`} </div ></> :<></>}
