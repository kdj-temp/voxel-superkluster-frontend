...
const TopFilterBar = ({ funcs, filterData, colormodesettle }) => {
    ...
    // const change_url=(cate , val)=>{
        
    //     // history.replace('/explore'+"?"+(localStorage.getItem('categoryUrl')?`category=${localStorage.getItem('categoryUrl')}`:'') + (localStorage.getItem('sortUrl')?`&sort=${localStorage.getItem('sortUrl')}`:''));
    //     const state = { 'page_id': 1, 'user_id': 5 }
    //     history.push('./explore?color=blue')
    //     let currentUrlParams = new URLSearchParams(window.location.search);
    //     currentUrlParams.set('page', 'ss');
    //     history.push(window.location.pathname + "?" + currentUrlParams.toString());

    //     console.log(currentUrlParams.get('page')) ;

    //     // console.log('history1',history.location.pathname) ;
    //     // history.replace('/explore'+"?filter="+filter);
    //     console.log('history2',history.location.search ,window.location.href) ;
    // }
    ...
    const handleCategoryChange = (value) => {
        ...
        // change_url('categoryUrl' , value) ;
        let currentUrlParams = new URLSearchParams(window.location.search);
        currentUrlParams.set('categoryUrl', value);
        history.push(window.location.pathname + "?" + currentUrlParams.toString());
        // console.log('category_url', isHistoryUrl) ;
    }
    ...
    const priceRangeDiv = (
        <div style={{ width: 300, height: 120 }}>
            <RangeDiv>
                {/* <TokenImgDiv>
                    <TokenSubDiv>
                        {isDefaultCurrency == '1' ? <img src="/img/VXL.png" style={{ ...styledImg, margin: '0px 5px' }} /> : <FaDollarSign/> }
                    </TokenSubDiv>
                </TokenImgDiv> */}
                <TokenSelectDiv>
    ...
    const showRangeValue = <>{isDefaultCurrency == '1' ? <img src="/img/VXL.png" style={{ ...styledImg, margin: '0px 5px' }} /> : isDefaultCurrency == '2' ? <FaDollarSign/>:<img src="/img/icons/ethIcon.png" style={{ ...styledImg, margin: '0px 5px' }} /> }<span style={{ color: '#d9d9d9', margin: '0px 3px' }}>|</span> </>;
