...
import { filter } from "online-3d-viewer";
...
const ColumnNewRedux = ({ filterData }) => {
  ...
  //     setSearchKey(state.value ? state.value : '');
  //   }
  // }, [state])

  useEffect(()=>{
    // console.log(localStorage.getItem('searchValue'),'serachValue') ;
  ...
  useEffect(() => {
    if(nfts) {
      let tmp = nfts.data  ;
      // console.log(nfts.data ,'nft.data' , tmp) ;
      // if(filterData.order_type == 4) tmp = tmp.sort((a,b)=>
      //   // a.price - b.price
      //   (b.top_bid?b.top_bid:b.price) - (a.top_bid?a.top_bid:a.price) 
      // ) ;
      // if(filterData.order_type == 3) {
      //   tmp = tmp.sort((a,b)=> (((a.top_bid?a.top_bid:a.price)==null || (a.top_bid?a.top_bid:a.price)== 0) ?Number.MAX_SAFE_INTEGER:(a.top_bid?a.top_bid:a.price)) - (((b.top_bid?b.top_bid:b.price) == null || (b.top_bid?b.top_bid:b.price) ==0) ? Number.MAX_SAFE_INTEGER:((b.top_bid?b.top_bid:b.price)))) ;
      // }
      setTempArr(tmp) ;
      // setUsdPrice(nfts.usdPrice) ;
      setEthPrice(nfts.ethUsdPrice);
      localStorage.setItem('usdPrice' , nfts.usdPrice) ;
      // console.log(nfts.usdPrice,'nfts.usdPrice') ;
      if (nfts.meta) {
  ...
  useEffect(() => {
    ...
    // change_url(filterData , isPageNumber, isPageSize, isSearchKey) ;
  ...
  const handlePageEvent = (pageNumber) => {
    // change_url(pageNumber,pageNumber,pageNumber,pageNumber) ;
    let currentUrlParams = new URLSearchParams(window.location.search) ;
    currentUrlParams.set('PageCurrent', pageNumber) ;
    history.push(window.location.pathname + "?" + currentUrlParams.toString()) ;
    setPageNumber(pageNumber) ;
    goToTop() ;
    // getNftItems(pageNumber, 25, filterData, '');
    // setLoading(true);
    }
  ...
  const onShowSizeChange = (current, pageSize) => {
    ...
    // getNftItems(current, pageSize, filterData, '');
  }

  const getNftItems = (pageNumber, pageSize, filterData, searchKey) => {
    // console.log(searchKey ,'filter-data') ;
    let tmp =[] ;
  ...
  return (
    <div className="row" style={{ padding: 10, justifyContent:'space-between' }}>
      {/* <div className="text-center" style={{marginBottom:'10px'}}>
        <Pagination showQuickJumper defaultCurrent={1} current={isPageNumber} total={isTotalNum} defaultPageSize={25} pageSizeOptions={[15, 25, 50, 100]} onChange={handlePageEvent} onShowSizeChange={onShowSizeChange} />
      </div> */}
      {
        localStorage.setItem('rim' , JSON.stringify(tempArr)) 
      }
      {
        // console.log(tempArr,'offline',UsdPrice)
      }
