...
const NftCard = ({
  ...
  const blockchainData = {
    5: { name: 'Goerli-Testnet', logo: '/img/blockchain/goerli.png'},
    1: { name: 'Ethereum', logo: '/img/blockchain/ethereum.png'},
    137: { name: 'Polygon', logo: '/img/blockchain/polygon.png'},
    42161: { name: 'Arbitrum', logo: '/img/blockchain/arbitrum.png'},
    56: { name: 'Binance Smart Chain', logo: '/img/blockchain/binance.png'},
    250: {name: 'Fantom', logo: '/img/blockchain/fantom.png'},
    43114: { name: 'Avalanche', logo: '/img/blockchain/avalanche.png'}
  };
            ...
            <div className="nft__item m-0 nftcardpage" style={{ position: 'relative' ,paddingRight:'4px',paddingLeft:'4px',paddingTop:'0px', height:'100%' }}>
              ...
              <a className="nftcardA" onClick={(e)=>openItems(e, nft ,nft.id)} href={`/ItemDetail/${nft.id}`} style={{height:'100%' }}>
                ...
                <a className="author_list_pp hoverauthor" style={{ width: 40, height: 40,marginTop:'6px' ,marginLeft:'6px'}} href = {`collection-detail/${nft.collection.link}`}>
                  <span>
                      <LazyLoadImage effect="opacity" src={nft && nft.collection && nft.collection.avatar ? nft.collection.avatar : "/img/collections/coll-1.jpg" } afterLoad={() => setLoadImgStatus(true)} alt="" />
                      {
                  ...
                  <Outer>
                      {
                        isSrcType == "video/mp4" || isSrcType == "audio/mpeg" || isSrcType == "application/octet-stream" ?
                          isSrcType == 'audio/mpeg' && <>
                            ...
                            <LazyLoadImage 
                              effect="opacity"
                              src={nft.image_preview ? (nft.image.slice(-3).toLowerCase() == "gif" ? nft.image:nft.image_preview) : (nft.image ? nft.image : (nft.raw_image? nft.raw_image : (nft.blob_raw_image)? nft.blob_raw_image:"/img/collections/coll-item-1.jpg"))}
                    ...
                    nft && nft.sale_type == 2 && nft.auction_end_process && nft.auction_end_process == true && (nft && nft.top_bid && nft.top_bid != "null") ?
                      <CardBottom >
                        ...
                        <div className="nItemSale">
                          ...
                          {nft && nft.top_bid && nft.top_bid != "null" ? <><div className="usd_color"><img style={{ width: 13, height: 16, marginBottom: 5 }} src="/img/icons/ethIcon.png" /> {`${usd_price_set_eth(nft.top_bid)} ($${usd_price_set_usd(nft.to_bid)})`} </div></> : <></>}                          
                      ...
                      <CardBottom >
                        <div className="nItemSale" style={{ width: 'max-content' }}>
                            ...
                            <div className="mobile-margin">
                              ...
                              {nft && nft.price && nft.price != "null"  && nft.price > 0  ? <><div className="usd_color"><img style={{ width: 16, height: 16, marginBottom: 5 }} src="/img/icons/ethIcon.png" /> {`${usd_price_set_eth(nft.price)} ($${usd_price_set_usd(nft.price)})`} </div ></> :<></>}
                            </div>
